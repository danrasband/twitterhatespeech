...
object TwitterSelectedTags {
  type TweetData = Tuple12[Long, String, Date, String, Boolean, Boolean, Int, Int, Boolean, String, Tuple2[String, String], Long]
  ...
  def main(args: Array[String]) {
    ...
    tweetStream.foreachRDD((rdd, time) => {
      ...
      if (count_ttl.value > 0) {
        ...
        // val tweetRDD = rdd.filter(status => status.getLang == "en")
        val tweetRDD = rdd.map(status => (status.getId,
        ...
        tweetRDD.foreach(tweet => sendToKafka("tweets-v4", tweet))

        val count = tweetRDD.count()
        numTweetsCollected += count
        println("Number of tweets collected this round: %s".format(count))
        println("Number of tweets collected in total: %s".format(numTweetsCollected))
      }
    })
