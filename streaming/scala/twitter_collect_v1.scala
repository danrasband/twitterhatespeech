...
object TwitterSelectedTags {
  ...
  def main(args: Array[String]) {
    ...
    println("USERS CHECK: %s".format(users_bc.value))
      ...
      if (count_ttl > 0) {
        val userRDD = rdd.filter(status => status.getLang == "en")
                         .map(status => (status.getId,
                                         ...
                                         status.getText))
                         .filter{case (tid, sn, text) => checkUser(sn) > 0}
        ...
        val vocabRDD = rdd.filter(status => status.getLang == "en")
                          .map(status => (status.getId,
                                          ...
                                          checkUser(status.getUser.getScreenName)))
                          .filter{case (tid, sn, text, cnt, usr) => cnt > 0}
                          .coalesce(partitionsEachInterval)
                          .cache()
